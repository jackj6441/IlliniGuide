...
| Benchmark harness + interview notes | Implemented | Measured on ICRN H200 with Qwen2.5-7B-Instruct: streaming p50 TTFT **55 ms** vs blocking p50 TTFT 472 ms at concurrency 10 (**9× perceived-latency reduction**); median total latency ~475 ms on both endpoints (throughput unchanged). See `docs/interview_notes_vllm.md` for full percentile breakdown, resume bullet, and 16 L1/L2/L3 Q&A grounded in the measurements. |
