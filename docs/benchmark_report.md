...
| Semantic pgvector, current corpus + curated career tags + scope guard | 367 source-tagged courses; 1,057 MiniLM 384-d chunks; 22 RAG evidence cases | 20/22 (90.9%) | 8/10 (80.0%) | 20/22 (90.9%) | 6/8 (75.0%) | 4/4 (100.0%) |
| Keyword chunk baseline, current corpus + curated career tags + scope guard | Same current corpus; 22 RAG evidence cases | 19/22 (86.4%) | 7/10 (70.0%) | 18/22 (81.8%) | 3/8 (37.5%) | 4/4 (100.0%) |
...
`20260714T020752Z-db49cad`. The current tag-seeded runs are
`20260714T024007Z-d9c4569` (semantic) and `20260714T024037Z-d9c4569`
(keyword), with embedding-ingestion manifest
`backend/artifacts/embedding_ingestion/20260714T023923Z-d9c4569/manifest.json`.
...
so **40.9% and 20.0% are not directly comparable quality claims**. The current
tag-seeded run improves unfiltered semantic Recall@3 to 8/10 (80.0%) while
keeping all four unsupported cases safe. On the production RAG path, all 12
direct-ID lookup cases retrieved the requested course, and invented ``ECE 999``
returned no evidence. This is still retrieval evidence, not answer quality.

These results **forbid** a “92% answer relevance” claim. Official-description
enrichment reduced empty courses from 92/360 to 1/367, direct-ID routing made
exact course lookup behavior defensible, and the manually curated
`gpu_programming` tag recovered the GPU/CUDA paraphrase. The remaining quality
work is broader topic coverage and reranking calibration; semantic section
correctness is now 6/8 (75.0%) but is not a generation-quality score. The
scope guard rejects all four unsupported cases (4/4 safety) without weakening
the supported-query result.
