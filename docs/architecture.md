...
| Observability | Partial | `/metrics` exposes application request/error counters, HTTP/tool/retrieval/LLM latency, and streaming TTFT; `scripts/gpu_sampler.py` provides a timestamped `nvidia-smi` fallback. Same-window GPU runs, scraping, and Grafana are not yet verified. |
